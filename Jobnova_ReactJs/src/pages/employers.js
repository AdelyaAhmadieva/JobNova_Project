...
export default function Employers(){
   ...
   const [employers, setEmployers] = useState([]);
    async function getEmployers(){
        try {
            const response = (await axios.get("http://localhost:5259/employers", {
                headers: {
                            ...
                            <div className="col-lg-3 col-md-4 col-sm-6 col-12" key={index}>
                                <div className="employer-card position-relative bg-white rounded shadow p-4 mt-3">
                                    ...
                                    <div className="content mt-3">
                                        <Link to={`/employer-profile/${item.id}`} className="title text-dark h5">{item.employerName}</Link>
                                    ...
                                    <ul className="list-unstyled d-flex justify-content-between align-items-center border-top mt-3 pt-3 mb-0">
                                        <li className="text-muted d-inline-flex align-items-center"><FiMapPin className="fea icon-sm me-1 align-middle"/>{item.address}</li>
                                        <li className="list-inline-item text-primary fw-medium">{item.vacancies.length} Jobs</li>
                                    </ul>
                                </div>
                            </div>
                        )
